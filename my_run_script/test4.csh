...
cd code
foreach f (lat_hgt.ncl lat_lon_sreg.ncl lat_lon_lreg.ncl) #strength_position_taux.ncl) #eof_regression_sfc_monthly.ncl) #(eof_regression_sfc_seasonal.ncl) #(enso_regression_sfc_monthly.ncl)#strength_position_taux.ncl) #lat_hgt.ncl) #lat_lon_lreg.ncl lat_lon_sreg.ncl)#eof_regression_sfc.ncl) #eof_sfc.ncl, eof_hgt.ncl lat_lon_lreg.ncl lat_lon_sreg.ncl) #(eof_hgt.ncl eof_sfc.ncl lat_hgt.ncl) #(*.ncl)
echo "---------------------------------------------"
echo $f "is running!"
echo "---------------------------------------------"
ncl<$f 
