...
while ( $i <= $ncase ) 
 ...
 while ($iy <= $end_year)
    ...
    set eam_files = ($RUN_FILE_DIR/*eam.h1*`printf "%04d" $ym1`-12*)
    set eam_files = ($eam_files $RUN_FILE_DIR/*eam.h1*`printf "%04d" $iy`*)
 
    if ( $iy < $end_year ) then  
      set eam_files = ($eam_files $RUN_FILE_DIR/*eam.h1*`printf "%04d" $yp1`-01*)
    endif 
    ...
    while ( $j <= $nvars )
      ...
      ncrcat -d time,"${timstr1}","${timstr2}" -v $var $eam_files ${SE_FILE} & 
     @ j++ 
    end 
    wait 
