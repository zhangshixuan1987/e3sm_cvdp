...
set exp_name   = v2.LR.amip.6hourly
set model_case = (v2.LR.amip_0101 v2.LR.amip_0201 v2.LR.amip_0301)
set ncase = $#model_case

set var2d_list = ("PRECC" "PRECT" "QFLX" "SHFLX" "TUQ" "TVQ")
...
while ( $i <= $ncase )
 ...
 while ($iy <= $end_year)
    ...
    set eam_files = ($RUN_FILE_DIR/*eam.h4*`printf "%04d" $ym1`-12*)
    set eam_files = ($eam_files $RUN_FILE_DIR/*eam.h4*`printf "%04d" $iy`*)
    if ( $iy < $end_year ) then
      set eam_files = ($eam_files $RUN_FILE_DIR/*eam.h4*`printf "%04d" $yp1`-01*)
    endif
    ...
    while ( $j <= $nvars )
      ...
      ncrcat -d time,"${timstr1}","${timstr2}",2 -v $var $eam_files ${SE_FILE} & 
     @ j++ 
    end 
    wait 
