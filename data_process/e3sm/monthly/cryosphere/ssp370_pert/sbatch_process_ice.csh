...
set run_name   = v2_1.SORRM.ssp370
set exp_name   = v2_1.SORRM.ssp370.fismf
set model_tag  = ( "0701" )
set model_ennm = ( "en00" )
set model_case =
foreach tag ( $model_tag )
  set model_case = ( ${model_case} ${run_name}_${tag}-fismf )
end
...
set MAP_FIL1  = /lcrc/group/acme/ac.szhang/acme_scratch/data/regrid_maps/E3SM_ocean_to_1x1_Rect_conserve.nc
...
while ( $i <= $ncase )
 ...
 echo $CASE_NAME
 set RUN_FILE_DIR = ${run_dir}/${CASE_NAME}/run #archive/ice/hist
 ...
 if ( -d $RUN_FILE_DIR ) then
  ...
  while ( $j <= $nvars )
   ...
   set ensr = $model_ennm[$i] 
   set SE_FILE = ${WORK_DIR}/SE_OCN/${exp_name}.${ensr}.${vou}.${time_tag}.nc
   set FV_FIL1 = ${WORK_DIR}/${exp_name}.0.5deg.${ensr}.${vou}.${time_tag}.nc
   set FV_FILE = ${WORK_DIR}/${exp_name}.${ensr}.${vou}.${time_tag}.nc

   rm -rvf $FV_FIL1 $FV_FILE ${WORK_DIR}/tmp_ocn_out.nc ${WORK_DIR}/tmp_ocn_out1.nc ${WORK_DIR}/tmp_ocn_out2.nc
   ncremap -i ${SE_FILE} -m ${MAP_FILE} -o ${FV_FIL1}
   ncremap -i ${FV_FIL1} -m ${MAP_FIL1} -o ${FV_FILE}
