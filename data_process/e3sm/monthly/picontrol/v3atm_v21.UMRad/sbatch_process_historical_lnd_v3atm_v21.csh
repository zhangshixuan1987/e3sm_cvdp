...
while ( $i <= $ncase )
 ...
 set iy = $start_year
 set RUN_FILE_DIR0 = /lcrc/group/e3sm/ac.wlin/E3SM_simulations/E3SMv3_dev/${CASE_NAME}/archive/lnd/hist
 set RUN_FILE_DIR1 = /lcrc/group/e3sm/ac.wlin/E3SM_simulations/E3SMv3_dev/${CASE_NAME}/run/

 set eam_files = 
 while ($iy <= $end_year)
   set eam_files = ($eam_files $RUN_FILE_DIR0/*elm.h0*`printf "%04d" $iy`* $RUN_FILE_DIR1/*elm.h0*`printf "%04d" $iy`*)
  @ iy ++
 end
