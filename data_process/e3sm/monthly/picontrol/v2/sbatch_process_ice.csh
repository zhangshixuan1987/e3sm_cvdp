...
while ( $i <= $ncase )
 ...
 set RUN_FILE_DIR = /lcrc/group/acme/ac.szhang/acme_scratch/data/CVDP_RGD/zstash/v2.LR.piControl/archive/ice/hist
 #ls $RUN_FILE_DIR
 ...
 if ( -d $RUN_FILE_DIR ) then
  ...
  set eam_files = 
  while ($iy <= $end_year)
   set file = $RUN_FILE_DIR/*mpassi.hist.am.timeSeriesStatsMonthly*`printf "%04d" $iy`*
   set nfile = $#file
   if ( $nfile < 12 )  then 
     echo "missing files " 
     echo $file 
