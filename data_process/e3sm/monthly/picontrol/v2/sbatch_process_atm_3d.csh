...
#set RUN_FILE_DIR = /lcrc/group/e3sm/ac.forsyth2/E3SMv2/${CASE_NAME}/archive/atm/hist
 set RUN_FILE_DIR = /lcrc/group/acme/ac.szhang/acme_scratch/data/CVDP_RGD/zstash/v2_atm/piControl/archive/atm/hist

 if ( -d $RUN_FILE_DIR ) then 

   set iy = 1
   while ($iy <= $nyr )
    @ yst = $start_year + ( $iy - 1 ) * $yint
    @ yed = $start_year + $iy * $yint - 1
    echo $yst $yed
